...
def eval(model, data_eval, voc_size, epoch, val=0, threshold1=0.8, threshold2=0.2):
    ...
    for step, input in enumerate(data_eval):
            ...
            if not args.sparse:
                ...
                if adm_idx == 0:
                    ...
                    if len(input) > 1:
                        y_old = np.zeros(voc_size[2])
                        y_old[adm[2]] = 1
                elif adm_idx < len(input)-1 and adm_idx!=0:
                    _, _, residual, _, _ = model(input[:adm_idx+1])
                    # prediction prod
                    representation_base += residual
                    y_pred_tmp = F.sigmoid(representation_base).detach().cpu().numpy()[0]
                    ...
                    y_old = np.zeros(voc_size[2])
                    y_old[adm[2]] = 1
                elif adm_idx==len(input)-1 and len(input)>1:
                    _, _, residual, _, _ = model(input[:adm_idx+1])
                    # prediction prod
                    representation_base += residual
                    y_pred_tmp = F.sigmoid(representation_base).detach().cpu().numpy()[0]
        ...
        if args.sparse and len(input)>1:
            y_gt = [y_gt[-1]]
        # print(np.array(y_gt).shape, np.array(y_pred).shape, np.array(y_pred_prob).shape)
...
def main():
    ...
    if args.embd_mode!="random":
        ...
        taxo_diag_embd = None
        taxo_pro_embd = None
    model = MICRON(voc_size, ddi_adj, emb_dim=args.dim, device=device,embd_mode=args.embd_mode, pro_taxo=args.pro_taxo,taxo_diag_embd=taxo_diag_embd, taxo_pro_embd=taxo_pro_embd, diag_ontology=sym_ontology,
    ...
    if args.test:
        ...
        # # range_temp = label_list.shape[1] if not args.sparse else 
