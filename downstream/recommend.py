...
# 读取disease跟proc的英文名
# icd_diag_path = '../data/DIAGNOSES_ICD.csv'
# icd_proc_path = '../data/PROCEDURES_ICD.csv'
# code2diag = {}
# code2proc = {}
...
def eval(model, eval_dataloader, voc_size, epoch, device, TOKENS, args):
    ...
    return ddi_rate, np.mean(ja), float(np.mean(prauc)), np.mean(avg_p), np.mean(avg_r), np.mean(avg_f1), med_cnt / visit_cnt
